...
`caller_phone`, `reason`, `created_at`, `resolved` bool) и шлёт Telegram-уведомление
**fire-and-forget ПОСЛЕ возврата `{ result }`** (не в `await` перед return — иначе
агент молчит, пока ждёт Telegram API; ошибка Telegram не роняет tool). Логика записи
и отчёта — в `lib/` (по образцу `lib/trips-search.ts`), route остаётся тонким.
...
- Верификация `ElevenLabs-Signature: t=<ts>,v0=<hmac>`: HMAC-SHA256 по `"{t}.{rawBody}"`,
  **raw body читается до JSON.parse**, сравнение timing-safe, анти-replay: `t` старше
  30 минут → отказ.
- Идемпотентность по `conversation_id`: **`INSERT ... ON CONFLICT (conversation_id)
  DO NOTHING`** на реальном UNIQUE-constraint (не SELECT-then-INSERT — гонка при
  параллельных ретраях ElevenLabs), в любом случае ответ 200.
...
## Производительность (для realtime-голоса латентность критична)

- **Регион Vercel**: закрепить `"regions": ["dub1"]` (Dublin) для `apps/admin` —
  Supabase живёт в eu-west-1 (Ирландия). Без этого каждый tool-call несёт
  трансатлантический RTT ×4–5 волн запросов (~450ms лишних, агент молчит).
  Проверить фактический регион деплоя ДО боевого звонка.
- В `searchTrips` (`lib/trips-search.ts`) перенести финальный запрос `tariff_periods`
  в общий `Promise.all` — минус одна последовательная волна на каждый search_trips.
- Cold-start: линия 24/7 с редкими звонками → первый tool-call разговора часто
  холодный. Держать route-хендлеры тонкими; тестировать DoD п.3 на холодной функции;
  в конфиге tools задать `pre_tool_speech` («секунду, проверяю…»).
- Позже (не блокер): ретеншен для `search_log` (94k+ строк); при росте объёма звонков —
  функциональный индекс на `normalize_stop_name(name_ro)` или materialized view
  для `v_interurban_v2_km_pairs`.
