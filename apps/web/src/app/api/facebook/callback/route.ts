...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    return new Response(`Facebook OAuth Error: ${error}`, { status: 400 });
  }
  ...
  const redirectUri = 'https://translux-web.vercel.app/api/facebook/callback';
  ...
  if (tokenData.error) {
    const html = `
      ...
      <body style="font-family: monospace; padding: 40px; background: #1e293b; color: #e2e8f0;">
        <h1 style="color: #ef4444;">Facebook OAuth — Ошибка</h1>
  ...
  // Build HTML with page tokens
  const pageRows = pages
    .map(
      (p: { id: string; name: string; access_token: string }) => `
      <tr>
        <td style="padding:8px; border-bottom:1px solid #334155;">${p.name}</td>
        <td style="padding:8px; border-bottom:1px solid #334155; font-size:12px;">${p.id}</td>
        <td style="padding:8px; border-bottom:1px solid #334155;">
          <textarea readonly style="width:100%; height:40px; background:#1e293b; color:#fff; border:1px solid #475569; padding:4px; font-size:11px;">${p.access_token}</textarea>
        </td>
      </tr>`,
    )
    .join('');
  ...
  const html = `
    ...
    <body style="font-family: monospace; padding: 40px; background: #1e293b; color: #e2e8f0;">
      ...
      <p>Найдено страниц: ${pages.length}</p>
      <div style="background: #0f172a; padding: 20px; border-radius: 8px; margin: 20px 0; overflow-x:auto;">
        <table style="width:100%; border-collapse:collapse;">
          ...
          <tbody>${pageRows || '<tr><td colspan="3" style="padding:8px;">Нет страниц</td></tr>'}</tbody>
        </table>
      </div>
      <p style="color: #94a3b8; font-size:12px;">User token: ${userToken.slice(0, 20)}...</p>
    </body>
    </html>
  `;
