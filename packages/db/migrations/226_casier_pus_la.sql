-- 226: get_casier_document — expune și momentul în care s-a înregistrat foaia de parcurs.
--
-- driver_cashin_receipts.created_at e ora reală a introducerii foii (timestamptz).
-- Nu coincide cu data_foaie (dcr.ziua): foaia se pune adesea din ajun — ex. foaia 947903
-- are ziua 11.07.2026, dar a fost introdusă pe 10.07.2026 la 13:47. De aceea se întoarce
-- ca un câmp separat (`pus_la`), nu lipit de data_foaie.
--
-- Adăugarea unui câmp nou în jsonb e retro-compatibilă: frontend-ul vechi îl ignoră.
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'ziua',             wg.kiosk_ziua,        -- ziua plății la casă
    'data_foaie',       wg.data_foaie,        -- ziua din /grafic (poate fi alta sau NULL)
    'pus_la',           wg.pus_la,            -- când s-a introdus foaia (NULL dacă nu e în /grafic)
    'plati',            wg.plati,
