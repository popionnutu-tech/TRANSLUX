-- 058_add_comment_to_driver_rows.sql
-- Returneaza comentariul Tomberon agregat per sofer in `rows`.
-- Util pentru evaluator: vede ce a notat soferul la depunere
-- (ex: "редуере", "80л ск бельцы", nume client, cod).
...
BEGIN
  ...
  incasare_via_grafic AS (
    SELECT r.driver_id, r.ziua AS report_ziua,
           tpr.cash, tpr.plati, tpr.lgotniki0, tpr.diagrama, tpr.rashodi, tpr.comment
