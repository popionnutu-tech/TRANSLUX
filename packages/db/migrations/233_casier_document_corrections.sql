-- 233: get_casier_document aplică corecțiile de sume (casier_amount_corrections) peste
-- valorile brute din tomberon și adaugă rândurile manuale (casier_manual_rows).
--
-- Bază: definiția din 226_casier_pus_la.sql. Schimbări:
--   - LEFT JOIN casier_amount_corrections pe (ziua, norm_nr); COALESCE(override, brut) pe sume + comentariu.
--   - câmp nou `corrected_fields` = array cu cheile corectate (override IS NOT NULL) → colorare per-celulă.
--   - câmpuri noi `is_manual`, `manual_id`, `norm_nr` pe rândurile tomberon.
--   - UNION ALL rândurile manuale ale zilei (is_manual=true, cash=0).
--
-- Retro-compatibilă: cu tabelele goale, sumele rămân brute, corrected_fields=[], niciun rând manual
-- → output identic cu 226 + câmpuri jsonb noi (ignorate de frontend-ul vechi). Cash-ul NU se corectează.
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'pus_la',           wc.pus_la,            -- când s-a introdus foaia (NULL dacă nu e în /grafic)
    'plati',            wc.plati,
