...
-- Versiunea 2: aplica override-uri evaluator, categoriseaza anomaliile (A/B/C),
-- include breakdown complet pe tipuri (numerar/card/lgotnici/dop_rashodi/comment/fiscal_nr),
-- returneaza statusul de confirmare al zilei.
--
-- Ordine logica:
-- 1. Pentru fiecare (receipt_nr, ziua) din Tomberon, verifica override:
--    - ASSIGN -> atribuie soferului din override
--    - IGNORE -> exclude complet
--    - Lipsa  -> matching automat (LATERAL pe driver_cashin_receipts)
-- 2. Anomaliile sunt platile FARA override care nu se potrivesc niciunui driver:
--    A) NO_FOAIE: receipt_nr cu format valid (7 cifre) dar absent din driver_cashin_receipts
--    B) DUPLICATE_FOAIE: receipt_nr apare la >=2 inregistrari in driver_cashin_receipts
--    C) INVALID_FORMAT: receipt_nr nu respecta pattern-ul ^[0-9]{7}$
...
BEGIN
  ...
  -- Confirmare zi (doar pentru ziua p_from = p_to)
  IF p_from = p_to THEN
...
COMMENT ON FUNCTION public.get_incasare_report IS
  'Raport incasari per sofer v2. Aplica override-uri evaluator (asignare/ignorare), categoriseaza anomaliile (NO_FOAIE / DUPLICATE_FOAIE / INVALID_FORMAT) cu breakdown complet pe tipuri, returneaza status confirmare zi.';
