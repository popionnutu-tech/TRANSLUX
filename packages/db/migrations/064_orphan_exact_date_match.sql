-- 064_orphan_exact_date_match.sql
-- Reguli noi pentru clasificarea încasărilor neasociate:
--   - alertă = nu există match exact (foaie + zi) în /grafic
--   - INVALID_FORMAT: număr ne-numeric
--   - NO_FOAIE: foaia n-a apărut niciodată în /grafic
--   - DATA_GRESITA (nou): foaia apare în /grafic, dar pe altă zi(le)
--
-- Anterior, dacă o foaie era în /grafic pe altă zi (grafic_count=1),
-- transacția kiosk pe altă zi nu era nici matched, nici anomalie -
-- pur și simplu dispărea. Acum apare ca DATA_GRESITA.
--
-- Pentru routes-view: păstrăm logica exact-match pe (foaie, ziua),
-- ceea ce era deja corect.
...
BEGIN
  ...
  routes_view AS (
    ...
    LEFT JOIN driver_cashin_receipts dcr ON dcr.driver_id = da.driver_id AND dcr.ziua = da.assignment_date
    LEFT JOIN counting_sessions cs ON cs.crm_route_id = da.crm_route_id AND cs.assignment_date = da.assignment_date
    LEFT JOIN tomberon_per_foaie tpf ON tpf.foaie_nr = dcr.receipt_nr AND tpf.ziua = da.assignment_date
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'foaie_nr', rs.foaie_nr, 'cancelled', rs.cancelled,
  ...
  -- 2. ORPHAN NUMERAR — neschimbat
  WITH cs_orphans AS (
    SELECT
      cs.id AS session_id, cs.crm_route_id,
      cr.dest_to_ro AS route_name, cr.time_nord,
      cs.assignment_date AS ziua,
      cs.driver_id, d.full_name AS driver_name,
  ...
  -- 3. ORPHAN INCASARE — REGULĂ NOUĂ: match exact pe (foaie, ziua)
  WITH ir AS (
  ...
  matches AS (
    SELECT ir.*,
      EXISTS(SELECT 1 FROM driver_cashin_receipts r
             WHERE r.receipt_nr = ir.receipt_nr AND r.ziua = ir.ziua) AS exact_match,
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'foaie_history',
        (SELECT COALESCE(jsonb_agg(jsonb_build_object(
                  'ziua', ev.ziua, 'driver_id', ev.driver_id,
                  'driver_name', ev.driver_name, 'source', ev.source
  ...
  FROM matches m
  WHERE NOT m.has_override AND NOT m.exact_match;
