-- 068_show_unassigned_scheduled_routes.sql
-- Pentru suburban: dacă o rută trebuia să meargă în acea zi (există
-- schedule activ ce conține DOW-ul) dar dispecerul nu a făcut
-- atribuire în /grafic, totuși o afișăm — cu câmpurile goale
-- (driver_name=null, foaie=null, etc). Așa nu se pierde din vedere.
-- Statusul devine 'no_driver'.
--
-- Adaugă și `row_key` în output (stable key când assignment_id e null).
...
BEGIN
  ...
  -- TOATE perechile (rută, zi) ce trebuie afișate
  base_pairs AS (
    ...
    WHERE da.assignment_date BETWEEN p_from AND p_to
      AND (
        ...
        OR NOT EXISTS (SELECT 1 FROM crm_route_schedules crs
                       WHERE crs.route_id = da.crm_route_id AND crs.active = true)
        OR EXISTS (
          SELECT 1 FROM crm_route_schedules crs
          WHERE crs.route_id = da.crm_route_id AND crs.active = true
    ...
    SELECT cr.id AS crm_route_id, gs.d::date AS ziua, NULL::uuid AS assignment_id
    FROM crm_routes cr
    ...
    WHERE cr.active = true AND cr.route_type = 'suburban'
      AND EXISTS (
        SELECT 1 FROM crm_route_schedules crs
        WHERE crs.route_id = cr.id AND crs.active = true
          AND EXTRACT(ISODOW FROM gs.d::date)::int = ANY(crs.days_of_week)
      )
      AND NOT EXISTS (
        SELECT 1 FROM daily_assignments da
        WHERE da.crm_route_id = cr.id AND da.assignment_date = gs.d::date
      )
  ),
  routes_view AS (
    SELECT
  ...
  -- (rest unchanged) - orphan_numerar, orphan_incasare, confirmation
  WITH cs_orphans AS (
