...
{ const { data } = await supa.from('lde_route_legs').select('from_locality,to_locality,km_real_median');
  if (data) for (const l of data) legs.set(legKey(l.from_locality, l.to_locality), Number(l.km_real_median)); }
const legObs = new Map(); // acumulează observații curate noi: key -> [km...]

function bridgeKm(a, b) { // cârpire săritură: leg-db → linie dreaptă
...
for (const day of DAYS) {
  for (const v of fleet) {
    ...
    if (r.npts > 0 && (r.km > 0 || r.stops.length)) {
      ...
      console.log(`  ${v.plate.padEnd(8)} ${day}  ${String(r.km).padStart(6)}km  v${r.vmax}  opriri:${r.stops.length}${r.patched>0?`  cârpit:${r.patched}km`:''}  ${chain.slice(0,90)}`);
    }
    if (WRITE && r.npts > 0) {
