...
export function DitheringShader({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const ctx = glRef.current, prog = programRef.current
      if (!ctx || !prog) return
      ctx.clear(ctx.COLOR_BUFFER_BIT)
      ctx.useProgram(prog)
      const loc = uniformLocationsRef.current
      if (loc.u_time) ctx.uniform1f(loc.u_time, t)
      if (loc.u_resolution) ctx.uniform2f(loc.u_resolution, sizeRef.current.w, sizeRef.current.h)
