...
export async function ensureDayMaterialized(date: string): Promise<void> {
  ...
  const rows: Array<Record<string, unknown>> = valid.map((s) => {
    ...
    return {
      ...
      vehicle_id: (u.has_weekly_template ? tplMap.get(key) : staticMap.get(key)) ?? null,
      status: 'planificat',
    };
  });

  // ── interurban/suburban: câte un rând per cursă activă; mașina din daily_assignments ──
  const { data: crm } = await db
    .from('crm_routes').select('id, route_type').eq('active', true);
  const { data: das } = await db
    .from('daily_assignments')
    .select('crm_route_id, retur_route_id, vehicle_id, vehicle_id_retur')
    .eq('assignment_date', date);
  const daVeh = new Map<number, string | null>();
  for (const d of das ?? []) {
    if (d.crm_route_id != null) daVeh.set(d.crm_route_id, d.vehicle_id);
    if (d.retur_route_id != null) daVeh.set(d.retur_route_id, d.vehicle_id_retur);
  }
  for (const r of crm ?? []) {
...
export async function listZi(date: string, directions: string[] | null): Promise<AtribuireView[]> {
  ...
  let q = db.from('lde_atribuiri_zilnice')
    .select('id, date, direction, route_kind, factory_route_id, shift_number, crm_route_id, vehicle_id, status, verification_note, route_key')
    .eq('date', date);
  ...
  // etichete: rute uzină / curse crm / plăci
  ...
  const [frRes, crmRes, vehRes, tplRes] = await Promise.all([
...
export async function syncWriteThrough(date: string): Promise<number> {
  ...
  const { data } = await db.from('lde_atribuiri_zilnice')
    .select('crm_route_id, vehicle_id')
    .eq('date', date)
    .in('status', ['modificat_proactiv', 'modificat_reactiv'])
    .not('crm_route_id', 'is', null);
  let n = 0;
  for (const r of data ?? []) {
    if (await writeThroughCrm(date, r.crm_route_id as number, r.vehicle_id as string | null)) n++;
  }
  return n;
}
...
export async function atribuie(rowId: string, vehicleId: string | null, userId: string): Promise<AtribuireRow> {
  ...
  const { data: row } = await db.from('lde_atribuiri_zilnice')
    .select('id, date, direction, route_kind, crm_route_id, status, factory_route_id, shift_number, vehicle_id, verification_note, route_key')
  ...
  const status = row.date < today ? 'modificat_reactiv' : 'modificat_proactiv';

  const { data: upd, error } = await db.from('lde_atribuiri_zilnice')
    .update({ vehicle_id: vehicleId, status, changed_by: userId, changed_at: new Date().toISOString() })
    .eq('id', rowId)
    .select('id, date, direction, route_kind, factory_route_id, shift_number, crm_route_id, vehicle_id, status, verification_note, route_key')
    .single();
  if (error) throw new Error(error.message);

  // write-through pentru cursele din orar (grafic dispecer / site public)
  if (row.route_kind !== 'uzina' && row.crm_route_id != null) {
    await writeThroughCrm(row.date, row.crm_route_id, vehicleId);
  }
  return upd as AtribuireRow;
}
