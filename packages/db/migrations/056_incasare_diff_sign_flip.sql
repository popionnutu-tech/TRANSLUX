-- 056_incasare_diff_sign_flip.sql
-- Inversam semnul Delta:
--   Inainte: diff = numarare - (incasare + lgotnici + rashodi)
--     -> +446 = sofer datoreaza, -446 = platit in plus
--   Acum: diff = (incasare + lgotnici + rashodi) - numarare
--     -> -446 = sofer datoreaza, +446 = platit in plus
--
-- Status logic se actualizeaza in consecinta:
--   underpaid (datoreaza) cand diff < 0
--   overpaid (platit in plus) cand diff > 0
...
BEGIN
  ...
  merged AS (
    SELECT ad.driver_id,
           ...
           -- Diff inversat: incasare + lgotnici + rashodi - numarare
           --   negativ = sofer datoreaza
           --   pozitiv = platit in plus
