...
const COLS = [
  { key: 'ruta', title: 'Ruta (nume – start real)', w: 200, align: 'start' as const },
  { key: 'sofer', title: 'Cine (locuiește)', w: 175, align: 'start' as const },
...
export async function generateNavetaImage(rows: NavetaRow[], opts: { titlu: string; perioada: string; zileLucratoare: number }): Promise<Buffer> {
  ...
  rows.forEach((row, i) => {
    ...
    const cells: Record<string, { text: string; fill: string; bold?: boolean }> = {
      ruta: { text: truncText(fR, numeRuta, fsz, COLS[0].w * S - 10 * S), fill: '#222', bold: mare },
      sofer: { text: truncText(fR, row.sofer, fsz, COLS[1].w * S - 10 * S), fill: '#222' },
