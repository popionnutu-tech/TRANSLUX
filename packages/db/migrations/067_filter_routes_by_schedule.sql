-- 067_filter_routes_by_schedule.sql
-- Pentru rutele SUBURBAN, ascundem din raport assignment-urile create
-- de dispecer pentru zile în care ruta nu are programare în
-- crm_route_schedules.days_of_week. Se aplică DOAR pentru suburban
-- și DOAR dacă ruta are măcar un schedule activ; altfel afișăm normal.
-- Interurban — neschimbate (rulează în fiecare zi în general).
--
-- days_of_week e ARRAY[int] cu valori 1-7 (ISO: 1=Luni ... 7=Duminică).
-- Match cu EXTRACT(ISODOW FROM assignment_date)::int.
...
BEGIN
  ...
  routes_view AS (
    ...
    -- Filtru zile-de-săptămână: doar pentru suburban cu schedules existente.
    AND (
      ...
      OR EXISTS (
        ...
        WHERE crs.route_id = da.crm_route_id
          AND crs.active = true
