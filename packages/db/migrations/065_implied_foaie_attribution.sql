-- 065_implied_foaie_attribution.sql
-- Auto-atribuire (implied attribution): dacă șoferul X are atribuire pe ziua D2
-- dar fără foaie în /grafic, iar la casă apare un foaie F al cărui ultim posesor
-- în /grafic e tot X, atunci F se asociază automat cu acea atribuire (X, D2).
-- Plata nu mai apare ca orphan, iar în vederea Pe rute apare cu foaie_source='implied'.
--
-- Vezi și 064 (exact-match rule). Această migrație e on-top: include și exact-match
-- și implied. Doar tranzacțiile fără nici un fel de match (explicit, implied,
-- override) rămân orphan.
...
BEGIN
  ...
  implied_raw AS (
    ...
    JOIN foaie_last_owner flo
      ON flo.receipt_nr = t.sofer_id AND flo.driver_id = da.driver_id
    WHERE NOT EXISTS (
      SELECT 1 FROM driver_cashin_receipts dcr
      WHERE dcr.driver_id = da.driver_id AND dcr.ziua = da.assignment_date
    )
    AND NOT EXISTS (
      SELECT 1 FROM driver_cashin_receipts dcr_other
      WHERE dcr_other.receipt_nr = t.sofer_id
      AND dcr_other.ziua = da.assignment_date
    )
  ),
    ...
    SELECT ir.*,
      ...
      EXISTS(
        ...
        AND NOT EXISTS (SELECT 1 FROM driver_cashin_receipts dcr_other
                        WHERE dcr_other.receipt_nr = ir.receipt_nr
                        AND dcr_other.ziua = ir.ziua)
      ) AS implied_match,
