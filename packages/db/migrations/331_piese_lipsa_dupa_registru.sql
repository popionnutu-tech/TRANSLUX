-- 331: Lipsa se măsoară față de REGISTRU, nu doar față de straturile FIFO.
--
-- Reparație la migr. 330, găsită la review înainte ca ecranul să ajungă la oameni.
--
-- Migr. 330 declara „stoc insuficient" atunci când straturile FIFO nu acopereau cantitatea. Dar disponibilul
-- FIFO și stocul din registru (`piese_current_stock.qty` = Σ qty_delta) se despart definitiv după PRIMA
-- eliberare scurtă: aceea nu produce niciun rând în `piese_fifo_alloc`, deci scade registrul fără să scadă
-- disponibilul. De atunci înainte, disponibilul e mereu mai mare decât registrul, exact cu cât s-a eliberat
-- în gol.
--
-- Ce însemna asta în practică — chiar scenariul incidentului 417/418:
--     registru  0, disponibil  0
--     eliberare 10 cu acord  →  registru −10, disponibil  0   (întrebarea apare, corect)
--     recepție  10           →  registru   0, disponibil 10
--     eliberare 10           →  NICIO ÎNTREBARE. registru −10, disponibil 0
-- Cele zece bucăți sosite fizic pleacă a doua oară, iar depozitul poate coborî oricât de adânc în minus
-- fără să mai vadă vreodată dialogul. Garda ar fi prins prima abatere și ar fi tăcut la toate următoarele.
--
-- Pe baza reală există azi exact o piesă în starea asta: 21068 la Briceni, registru −1, disponibil 0 —
-- piesa din incident. Deci nu e un scenariu teoretic, e cel care tocmai s-a întâmplat.
--
-- Reparația: lipsa e cea mai mare dintre cele două deficite. Straturile FIFO rămân în calcul — o piesă cu
-- registru bun dar fără straturi înseamnă alocări pierdute, adică tot o problemă care merită oprită.
...
BEGIN
  ...
  WITH cerute AS (
    ...
    SELECT c.part_id, c.cerut,
           COALESCE(s.qty, 0) AS stoc,
           GREATEST(LEAST(COALESCE(f.disponibil_fifo, 0), COALESCE(s.qty, 0)), 0) AS disponibil
      FROM cerute c
      LEFT JOIN fifo f ON f.part_id = c.part_id
      LEFT JOIN piese_current_stock s ON s.part_id = c.part_id AND s.warehouse_id = p_wh
...
BEGIN
  ...
  FOR ln IN SELECT e FROM jsonb_array_elements(p_lines) e ORDER BY (e->>'part_id')::bigint LOOP
    ...
    SELECT COALESCE(s.qty, 0) INTO v_ledger FROM piese_current_stock s
      WHERE s.part_id = v_part AND s.warehouse_id = p_wh;
    ...
    END LOOP;
    v_short := GREATEST(v_need, v_qty - GREATEST(v_ledger, 0));
...
BEGIN
  ...
  FOR ln IN SELECT e FROM jsonb_array_elements(p_lines) e ORDER BY (e->>'part_id')::bigint LOOP
    ...
    IF NOT FOUND THEN RAISE EXCEPTION 'BAD_PART'; END IF;
    SELECT COALESCE(s.qty, 0) INTO v_ledger FROM piese_current_stock s
      WHERE s.part_id = v_part AND s.warehouse_id = d.warehouse_id;
    v_ledger := COALESCE(v_ledger, 0);
    v_need := v_qty; v_total := 0; alloc := '[]'::jsonb;
    FOR lyr IN
      SELECT r.id, r.unit_cost,
