-- 055_incasare_align_to_trip_day.sql
-- BUG: raportul se alinia la ziua_depunerii la Tomberon, nu la ziua cursei.
-- Soferii depun cu intirziere de 1-N zile, deci numararea zilei X se compara
-- cu depunerea de la o cursa anterioara, ceea ce produce date inconsecvente.
--
-- Fix: aliniem la ziua_cursei (driver_cashin_receipts.ziua). Pentru fiecare
-- foaie atribuita in /grafic pe ziua X, agregam TOATE platile Tomberon cu
-- acel receipt_nr (oricind a fost depusa).
--
-- Override-urile ASSIGN raman aliniate la ziua_depunerii din override
-- (pentru ca nu cunoastem ziua cursei pentru plati orphane).
-- Override-urile IGNORE exclud plata complet.
...
BEGIN
  ...
  -- Plati Tomberon agregate per receipt_nr (peste toate zilele de depunere)
  tomberon_per_receipt AS (
    ...
    WHERE NOT EXISTS (
      ...
      WHERE ovr.receipt_nr = t.sofer_id
        AND ovr.ziua = t.ziua
        AND ovr.action = 'IGNORE'
    )
    GROUP BY t.sofer_id
  ),
  -- Incasarile aliniate la ziua_cursei: pornesc de la /grafic, attach Tomberon
  incasare_via_grafic AS (
    SELECT r.driver_id,
           r.ziua AS report_ziua,
    ...
    WHERE r.ziua BETWEEN p_from AND p_to
      AND NOT EXISTS (
        -- Daca exista override ASSIGN pe orice depunere a acestui receipt_nr,
        -- override-ul precede /grafic (re-atribuie plata)
        SELECT 1 FROM tomberon_payment_overrides ovr
        WHERE ovr.receipt_nr = r.receipt_nr AND ovr.action = 'ASSIGN'
      )
  ),
  -- Override-urile ASSIGN aliniate la ziua_depunerii (a override-ului)
  incasare_via_override AS (
    SELECT ovr.driver_id,
           ovr.ziua AS report_ziua,
           SUM(COALESCE(t.suma_numerar,0))::numeric AS cash,
           COUNT(*)::int                            AS plati,
           SUM(COALESCE(t.lgotniki_count,0))::numeric AS lgotniki0,
           SUM(COALESCE(t.lgotniki_suma,0))::numeric AS diagrama,
    ...
    WHERE ovr.action = 'ASSIGN'
      AND ovr.ziua BETWEEN p_from AND p_to
    GROUP BY ovr.driver_id, ovr.ziua
  ),
  incasare_combined AS (
    SELECT * FROM incasare_via_grafic
    UNION ALL
    SELECT * FROM incasare_via_override
  ),
  incasare_aggregated AS (
    ...
    FROM incasare_combined
    GROUP BY driver_id
  ),
  numarare_aggregated AS (
    SELECT driver_id, SUM(suma) AS suma FROM numarare GROUP BY driver_id
  ),
  all_drivers AS (
    SELECT driver_id FROM numarare_aggregated
    UNION
  ...
  -- Anomalii: ramin aliniate la ziua_depunerii la Tomberon (nu se cunoaste ziua cursei)
  WITH ir AS (
    SELECT t.sofer_id AS receipt_nr,
           t.ziua,
           SUM(COALESCE(t.suma_numerar,0))::numeric AS cash,
...
COMMENT ON FUNCTION public.get_incasare_report IS
  'Raport incasari per sofer v5. Aliniere la ZIUA CURSEI (driver_cashin_receipts.ziua), nu la ziua_depunerii. Pentru fiecare foaie din /grafic pe ziua X, agregam toate platile Tomberon cu acel receipt_nr (oricind depuse). Anomaliile raman aliniate la ziua depunerii (foile orphane fara mapping).';
