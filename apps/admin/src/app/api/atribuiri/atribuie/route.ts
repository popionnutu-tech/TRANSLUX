...
import { atribuie, directionOfRow } from '@/lib/atribuiri/core';
...
export async function POST(req: NextRequest) {
  ...
  const body = await req.json().catch(() => null) as { rowId?: string; vehicleId?: string | null } | null;
  ...
  try {
    const row = await atribuie(body.rowId, body.vehicleId ?? null, auth.user.id);
