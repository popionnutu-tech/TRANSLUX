-- 062_grafic_report.sql
-- Raport orientat pe RUTĂ (din /grafic) în loc de pe șofer.
--
-- Pentru fiecare zi din perioadă:
--   - `routes`: lista TUTUROR atribuirilor din daily_assignments, cu
--     șofer, mașină, foaie de parcurs, numărare și incasare atașate.
--   - `orphan_numerar`: counting_sessions care nu pot fi legate de un
--     șofer sau de o atribuire din /grafic.
--   - `orphan_incasare`: plăți de la casă a căror foaie nu se găsește
--     în /grafic (echivalentul vechilor `anomalies`).
--   - `confirmation`: dacă ziua e confirmată de evaluator (doar pentru
--     filtru pe o singură zi).
...
BEGIN
  ----------------------------------------------------------------------
  -- 1. ROUTES — lista completă a /grafic pentru perioadă
  ----------------------------------------------------------------------
  WITH tomberon_per_foaie AS (
    SELECT
  ...
  routes_view AS (
    SELECT
      ...
      da.crm_route_id                        AS crm_route_id,
      da.assignment_date                     AS ziua,
      cr.dest_to_ro                          AS route_name,
      cr.time_nord                           AS time_nord,
      COALESCE(cr_retur.time_chisinau, cr.time_chisinau) AS time_chisinau,
      da.driver_id                           AS driver_id,
      d.full_name                            AS driver_name,
      v.plate_number                         AS vehicle_plate,
      vr.plate_number                        AS vehicle_plate_retur,
      dcr.receipt_nr                         AS foaie_nr,
      cs.id                                  AS counting_session_id,
      cs.tur_total_lei                       AS tur_total_lei,
      cs.retur_total_lei                     AS retur_total_lei,
      ...
      tpf.comment                            AS incasare_comment,
      COALESCE(tpf.plati, 0)                 AS plati,
      tpf.fiscal_nrs                         AS fiscal_nrs,
      EXISTS (
    ...
    LEFT JOIN counting_sessions cs
           ON cs.crm_route_id = da.crm_route_id AND cs.assignment_date = da.assignment_date
    LEFT JOIN tomberon_per_foaie tpf
           ON tpf.foaie_nr = dcr.receipt_nr AND tpf.ziua = da.assignment_date
    WHERE da.assignment_date BETWEEN p_from AND p_to
  ),
  routes_status AS (
    SELECT rv.*,
      ((rv.incasare_numerar + rv.incasare_diagrama)
        + rv.ligotniki0_suma + rv.dop_rashodi
        - rv.numarare_lei) AS diff,
      CASE
        ...
        WHEN rv.numarare_lei > 0 AND ABS(
          (rv.incasare_numerar + rv.incasare_diagrama)
          + rv.ligotniki0_suma + rv.dop_rashodi
          - rv.numarare_lei
        ) / rv.numarare_lei <= 0.05                         THEN 'ok'
        WHEN ((rv.incasare_numerar + rv.incasare_diagrama)
          + rv.ligotniki0_suma + rv.dop_rashodi)
          < rv.numarare_lei                                  THEN 'underpaid'
        ELSE 'overpaid'
      END AS status
    FROM routes_view rv
  )
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'crm_route_id',          rs.crm_route_id,
      'ziua',                  rs.ziua,
      'route_name',            rs.route_name,
      'time_nord',             rs.time_nord,
      'time_chisinau',         rs.time_chisinau,
      'driver_id',             rs.driver_id,
      'driver_name',           rs.driver_name,
      'vehicle_plate',         rs.vehicle_plate,
      'vehicle_plate_retur',   rs.vehicle_plate_retur,
      'foaie_nr',              rs.foaie_nr,
      'cancelled',             rs.cancelled,
      'counting_session_id',   rs.counting_session_id,
      'counting_status',       rs.counting_status,
      'tur_total_lei',         rs.tur_total_lei,
      'retur_total_lei',       rs.retur_total_lei,
      ...
      'plati',                 rs.plati,
      'comment',               rs.incasare_comment,
      'fiscal_nrs',            rs.fiscal_nrs,
      'diff',                  ROUND(rs.diff, 2),
      'status',                rs.status
  ...
  ----------------------------------------------------------------------
  -- 2. ORPHAN NUMERAR — counting_sessions fără șofer sau fără atribuire
  ----------------------------------------------------------------------
  WITH cs_orphans AS (
    SELECT
      cs.id                                AS session_id,
      cs.crm_route_id                       AS crm_route_id,
      cr.dest_to_ro                         AS route_name,
      cr.time_nord                          AS time_nord,
      cs.assignment_date                    AS ziua,
      cs.driver_id                          AS driver_id,
      d.full_name                           AS driver_name,
      cs.tur_total_lei                      AS tur_total_lei,
      cs.retur_total_lei                    AS retur_total_lei,
      ...
      CASE
        ...
        WHEN NOT EXISTS (
          ...
          WHERE da.crm_route_id = cs.crm_route_id
            AND da.assignment_date = cs.assignment_date
        ) THEN 'no_grafic'
        ELSE NULL
      END AS reason
    ...
    WHERE cs.assignment_date BETWEEN p_from AND p_to
      AND (COALESCE(cs.tur_total_lei,0) + COALESCE(cs.retur_total_lei,0)) > 0
  )
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'session_id',   o.session_id,
      'crm_route_id', o.crm_route_id,
      'route_name',   o.route_name,
      'time_nord',    o.time_nord,
      'ziua',         o.ziua,
      'driver_id',    o.driver_id,
      'driver_name',  o.driver_name,
      'tur_total_lei',o.tur_total_lei,
      'retur_total_lei',o.retur_total_lei,
      'total_lei',    ROUND(o.total_lei, 2),
      'counting_status', o.counting_status,
      'reason',       o.reason
  ...
  ----------------------------------------------------------------------
  -- 3. ORPHAN INCASARE — la fel cu vechiul `anomalies` (cu foaie_history)
  ----------------------------------------------------------------------
  WITH ir AS (
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'foaie_history',
        (SELECT COALESCE(jsonb_agg(jsonb_build_object(
                  ...
                  'source',      ev.source
                ) ORDER BY ev.ziua DESC, ev.prio), '[]'::jsonb)
         FROM (
      ...
      ELSE NULL END
    ) ORDER BY (m.cash + m.diagrama) DESC), '[]'::jsonb) INTO v_orphan_inc
  FROM matches m
  WHERE NOT m.has_override AND (NOT m.valid_format OR m.grafic_count = 0 OR m.grafic_count >= 2);

  ----------------------------------------------------------------------
  -- 4. CONFIRMATION (doar pentru o singură zi)
  ----------------------------------------------------------------------
  IF p_from = p_to THEN
