-- 069_redirect_kiosk_to_grafic_date.sql
-- Pentru fiecare plată din casă, calculăm "effective_ziua":
--   - Dacă foaia există în /grafic pe ziua plății -> ziua plății (exact match)
--   - Altfel, dacă foaia există în /grafic pe altă zi -> cea mai apropiată /grafic ziua
--   - Altfel -> NULL (rămâne NO_FOAIE orphan)
--
-- Plățile cu effective_ziua diferit de ziua plății se "mută" automat
-- la ziua corectă. Categoria DATA_GRESITA dispare — nu mai e alertă.
-- Orphan_incasare conține DOAR foi care n-au apărut nicăieri în /grafic
-- (NO_FOAIE) sau format invalid.
...
BEGIN
  ...
  -- DATA_GRESITA dispare: tranzactiile cu foaie in /grafic se redirectoneaza automat.
  WITH ir AS (
