-- 246: Un șofer poate avea 2 foi de parcurs pe zi (câte una per rută).
--
-- Problemă (raportată de operatorul terminalului Tomberon): când un șofer face
-- 2 curse cu 2 foi în aceeași zi, la noi încăpea o singură foaie —
-- UNIQUE (driver_id, ziua) pe driver_cashin_receipts + upsert care suprascria.
-- A doua foaie nu exista în baza noastră, deci tomberon-sync n-avea ce trimite.
--
-- Soluție: foaia se leagă (opțional) de rută — crm_route_id NULL = «pe toată
-- ziua» (rândurile istorice + atribuirile din evaluator, fără context de rută).
-- Unicitatea devine (driver, zi, rută), cu NULL tratat ca o singură valoare.
-- UNIQUE (receipt_nr) global rămâne — o foaie nu se refolosește niciodată.
--
-- get_grafic_report: explicit_attributions leagă acum foaia DOAR de ruta ei
-- (dacă e setată), în loc s-o propage pe toate atribuirile șoferului din zi.
-- Invarianți păstrați (review architecture-guardian):
--   * o foaie → o singură rută (effective_dedup, ca în migr. 239);
--   * o rută → o singură foaie (effective_route, nou) — altfel foaia zilei
--     (NULL) plus cea legată ar dubla banii aceleiași curse;
--   * foaie legată de o rută pe care șoferul n-o mai are azi (cursa mutată)
--     cade înapoi pe comportamentul «pe zi», ca banii să nu dispară din raport.
--
-- ATENȚIE ordine deploy: migrația asta NU șterge UNIQUE(driver_id, ziua) —
-- codul vechi de pe Vercel face upsert pe perechea asta până se termină
-- deploy-ul. Constraint-ul se șterge în 247, DUPĂ deploy-ul codului nou.
...
COMMENT ON COLUMN public.driver_cashin_receipts.crm_route_id IS
  'Ruta foii (opțional). NULL = foaia zilei, comportamentul istoric. Setat din /grafic sau mini-app atribuiri când șoferul are mai multe curse pe zi.';

-- COALESCE(-1): două rânduri fără rută pe același (șofer, zi) tot se resping.
-- Prefixul (driver_id, ziua) preia rolul de index al constraint-ului vechi.
CREATE UNIQUE INDEX IF NOT EXISTS uq_dcr_driver_ziua_ruta
  ON public.driver_cashin_receipts (driver_id, ziua, COALESCE(crm_route_id, -1));
...
-- ── get_casier_document (migr. 240): același principiu — foaia legată de rută
-- se leagă DOAR de atribuirea rutei ei, nu de o atribuire aleasă la întâmplare
-- dintre cele 2 ale șoferului. Restul funcției rămâne neschimbat.
