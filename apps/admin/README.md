...
- Скопировать ENV из текущего проекта: `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`,
  `AUTH_SECRET`, `CRON_SECRET`, `VOICE_API_KEY`, `VOICE_LLM_SECRET` (Bearer custom-LLM proxy; агент смотрит на проект translux-voice-llm/iad1, роут `/api/voice/custom-llm` здесь — fallback), `ANTHROPIC_API_KEY`, и т.д.
