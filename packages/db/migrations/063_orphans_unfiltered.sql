-- 063_orphans_unfiltered.sql
-- "Pe rute" rămâne filtrat pe perioada selectată.
-- Dar "nepuse" (orphan_numerar + orphan_incasare) trebuie să arate
-- TOATE elementele neasociate, indiferent de dată — fiindcă șoferul
-- poate depune banii cu zile întârziere și data depunerii nu reflectă
-- ziua reală a cursei. Listele se golesc pe măsură ce orphan-urile
-- sunt rezolvate (override sau /grafic completat).
...
BEGIN
  -- 1. ROUTES — filtrat pe perioadă (Pe rute)
  ...
  -- 2. ORPHAN NUMERAR — TOATE perioadele
  WITH cs_orphans AS (
    SELECT
  ...
  -- 3. ORPHAN INCASARE — TOATE perioadele
  WITH ir AS (
